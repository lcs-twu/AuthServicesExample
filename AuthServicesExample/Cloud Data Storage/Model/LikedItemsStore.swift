...
class LikedItemsStore: ObservableObject {
    
    // MARK: Stored properties
    @Published var likedItems = LikedItems()
    ...
    func refreshFromRemoteJSONSource() async {
        ...
        do {
            ...
            let decodedData = try decoder.decode(LikedItems.self, from: dataFromSheety)
            ...
            await MainActor.run {
                self.likedItems.likes = decodedData.likes
            }
    ...
    func loadFromLocalJSONSource() {
        ...
        if let url = Bundle.main.url(forResource: "likedItems", withExtension: "json") {
            do {
                
                let dataFromAppBundle = try Data(contentsOf: url)
                let decoder = JSONDecoder()
                let decodedData = try decoder.decode(LikedItems.self, from: dataFromAppBundle)
...
//let testStore = LikedItemsStore(loadFromRemote: false)
